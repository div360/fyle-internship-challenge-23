...
export class ApiService {
  ...
  getUser(githubUsername: string): Observable<UserDataRepresentation> {
    return this.httpClient.get<UserDataRepresentation>(
      `${this.baseUrl}/users/${githubUsername}`
    );
  }

  getStarReposCount(githubUsername: string): Observable<number> {
    return this.httpClient
      .get<UserDataRepresentation>(
        `${this.baseUrl}/users/${githubUsername}/starred`,
        {
          observe: 'response',
          params: { per_page: '1' },
        }
  ...
  getSearchedReposForUser( githubUsername: string, perPageCount: number, pageNumber: number, searchQuery: string): Observable<any> {
    const apiUrl = `https://api.github.com/search/repositories?q=user:${githubUsername}+${searchQuery}&per_page=${perPageCount}&page=${pageNumber}`;
    return this.httpClient.get(apiUrl, { observe: 'response' }).pipe(
      map((response) => response),
      catchError((error) => {
        if (error.status === 404) {
